...
class Appt:
    ...
    def __init__(self, day, begin, end, desc):
        ...
        if self.end < self.begin:
            raise ValueError("Appointment should end after it begins") # if the appointment ends before it begins
        self.desc = desc
        return
    ...
    def __lt__(self, other):
        ...
        """
        if self.end <= other.begin: # lt must NOT be defined in terms of gt(to prevent infinite loop)
            return True             # '=' is to avoid the border case, ie: A ends at 12pm, B begins at 12pm
        return False 
    ...
    def overlaps(self, other):
        ...
        Arguments:
                other: another Appt
        Returns:
                Ture iff one appointment time period overlaps the other      
        """
        if self > other:
            return False
        if self < other:
            return False
        return True # True iff they have same time period 
    ...
    def intersect(self, other, desc=""):
        ...
        if desc=="":
            desc = self.desc
        assert(self.overlaps(other))  # enter this code only when they overlap
        
        newbegin = max(self.begin, other.begin) # later of two begin times
        newend = min(self.end, other.end)       # earlier of two end times
        
        return Appt(newbegin.date(), newbegin.time(),
                    newend.time(), desc)
...
class Agenda:
    ...
    def normalize(self):
        """Normalize each agenda. Order appointments by begin time.
           Merge overlapping appointments.

           Returns:
              An agenda that contains merged busy appointments.
        """
        self.appts = self.appts.copy()   # a copy of current agenda 
        ...
        for appt in self.appts[1:]:
            ...
            end2 = appt.end
            text2 = appt.desc
            if begin2 > end: # not overlap case
                result.append(Appt(begin.date(), begin.time(), end.time(), text))
                begin, end, text = begin2, end2, text2
            else: # they did overlap
                end = max(end, end2)
                text = text + "; " + text2
        
        apptlast = Appt(begin.date(), begin.time(), end.time(), text)
        result.append(apptlast)
        return result
    ...
    def complement(self, freeblock):
        """Produce the complement of an agenda
           ...
           an appointment. For example, if this agenda
           is a set of appointments. Produce a new agenda
           of the times *not* in appointments in a given
           time period.
           Args:
        ...
        result = Agenda()
        normalized_schedule = self.normalize()
        for each_appt in normalized_schedule:
            if each_appt > freeblock: # Check if they overlap
                break                 # If not, ending the loop
            if each_appt < freeblock:
                continue              # Skip to the next iteration if they are on different day
            
            if each_appt.begin > current_time:
                result.append(Appt(each_appt.begin.date(), current_time.time(), each_appt.begin.time(), freeblock.desc))
            current_time = each_appt.end
            
        if current_time < freeblock.end:
            result.append(Appt(freeblock.begin.date(), current_time.time(), freeblock.end.time(), freeblock.desc))
    ...
    def intersect(self,other,desc=""): 
        ...
        result = Agenda()
        for each in self:
            for othereach in other: # iteration (__iter__) is used
                if each.overlaps(othereach):
                    result.append(each.intersect(othereach, desc))
