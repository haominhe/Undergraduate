...
Authors:  Haomin He
Consulted in design: Professor and GTFs
...
def read( file, min_length=3 ):
    ...
    for word in file:
        word = word.strip()
    # do nothing if word is too short or contain non-alphabetic characters
        if len(word) >= min_length and word.isalpha:   
            WORDSlst.append(word)
    WORDSlst = sorted(WORDSlst)  # Being sorted is most important for binary search
    WORDSlst.append('zzzzzzzzzzzzzzzz') # add a sentinel
...
def search( string ):
    ...
    while low <= high:
        mid = (high + low) // 2
        if string < WORDSlst[mid]:
            high = mid - 1
        elif string > WORDSlst[mid]:
            low = mid + 1
        else:
           return WORD
    
    if WORDSlst[low].startswith(string):
        return PREFIX
    else:
        return NO_MATCH
